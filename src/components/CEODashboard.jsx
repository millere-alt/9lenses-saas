...
function CEODashboard() {
  ...
  const stats = [
    { label: 'Active Assessments', value: '3', change: '+2 this month', icon: PlayCircle, color: 'emerald', gradient: 'from-blue-400 to-blue-500' },
    { label: 'Completed Assessments', value: '12', change: '+5 this quarter', icon: CheckCircle, color: 'teal', gradient: 'from-blue-400 to-cyan-500' },
    { label: 'Total Participants', value: '47', change: '8 pending invites', icon: Users, color: 'blue', gradient: 'from-blue-400 to-indigo-500' },
    { label: 'Avg Score', value: '6.8', change: '+0.3 vs last quarter', icon: TrendingUp, color: 'emerald', gradient: 'from-blue-500 to-blue-600' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-20 right-0 w-96 h-96 bg-gradient-to-br from-blue-200/30 to-blue-300/30 rounded-full blur-3xl animate-pulse-slow"></div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-500 text-white rounded-2xl shadow-lg hover:shadow-xl transition-all">
      ...
      <div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl overflow-hidden border border-gray-100">
          {recentActivity.map((activity, index) => (
            <div key={index} className="group p-6 hover:bg-gradient-to-r hover:from-blue-50 hover:to-blue-50 transition-all duration-300 border-b border-gray-100 last:border-0">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="font-bold text-gray-900 mb-1 group-hover:text-blue-600 transition-colors">{activity.type}</p>
                  <p className="text-gray-600 group-hover:text-gray-700 transition-colors">{activity.description}</p>
                </div>
                <span className="text-sm font-medium text-gray-500 group-hover:text-blue-600 whitespace-nowrap ml-4 transition-colors">{activity.time}</span>
              </div>
            </div>
          ))}
        </div>
